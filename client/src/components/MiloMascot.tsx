  ...
  return (
    <div className={`relative ${sizes[size]} ${className}`} aria-label="ไมโล แมวผู้ช่วย AI">
      <style>{`
        ...
        .milo-maneki-paw {
          animation: miloManeki 1.05s ease-in-out infinite;
      ...
      <div className="milo-art-float relative h-full w-full">
        ...
        <img src="/milo-mascot-reference.png" alt="ไมโล แมวผู้ช่วย AI" className="relative z-10 h-full w-full object-contain drop-shadow-xl" draggable={false} />
        {size !== "sm" && (
          <div className="milo-maneki-paw pointer-events-none absolute left-[7%] top-[19%] z-30" aria-hidden="true">
            <svg viewBox="0 0 120 150" className="h-[42%] w-auto overflow-visible drop-shadow-lg" role="presentation">
              <g>
                ...
                <circle cx="55" cy="43" r="4" fill="#ff7f86" /><circle cx="64" cy="39" r="4" fill="#ff7f86" /><circle cx="71" cy="45" r="4" fill="#ff7f86" />
              </g>
            </svg>
          </div>
        )}
