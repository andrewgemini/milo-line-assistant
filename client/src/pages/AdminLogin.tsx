  ...
  return (
    <div className="grid min-h-screen place-items-center bg-[#f4faf7] px-5">
      <div className="w-full max-w-md rounded-[2.2rem] border border-[#d2ebe5] bg-white p-8 text-center paper-shadow">
        <div className="mx-auto grid size-16 place-items-center rounded-2xl bg-[#e4f7f1] text-[#1c8c72]"><ShieldCheck className={`size-8 ${pending ? "animate-pulse" : ""}`} /></div>
        ...
        <form onSubmit={submit} className="mt-6 space-y-4 text-left">
          <div><label className="mb-1 block text-xs font-semibold text-[#284f47]">ชื่อผู้ใช้ (Username)</label><input type="text" autoComplete="username" value={username} onChange={event => setUsername(event.target.value)} placeholder="ชื่อผู้ใช้ผู้ดูแลระบบ" disabled={pending} className="h-11 w-full rounded-xl border border-[#cbe3dc] px-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#238f76]" /></div>
          <div><label className="mb-1 block text-xs font-semibold text-[#284f47]">รหัสผ่าน (Password)</label><div className="relative"><input type={showPassword ? "text" : "password"} autoComplete="current-password" value={password} onChange={event => setPassword(event.target.value)} placeholder="กรอกรหัสผ่านผู้ดูแลระบบ" disabled={pending} className="h-11 w-full rounded-xl border border-[#cbe3dc] pl-3.5 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-[#238f76]" /><button type="button" onClick={() => setShowPassword(value => !value)} className="absolute right-3 top-3 text-[#79a098] hover:text-[#238f76]">{showPassword ? "ซ่อน" : "ดู"}</button></div></div>
        ...
        <div className="mt-4 rounded-xl border border-[#e4f5ef] bg-[#f5fbf9] p-3 text-center"><p className="text-[11px] leading-5 text-[#5e877f]">ไม่มีการแสดง Username หรือรหัสผ่านเริ่มต้นบนหน้า Login</p></div>
