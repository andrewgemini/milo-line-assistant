...
export default function Dashboard() {
  ...
  return <div className={`relative min-h-screen bg-[#f5fcfa] text-[#245851] ${!isProductionSite ? "preview-surface" : ""}`}>
    ...
    <main className="relative lg:ml-64" aria-busy={contentMutationPending}>
      ...
      <header className="sticky top-0 z-20 flex items-center justify-between border-b border-[#d8ede8] bg-[#f5fcfa]/90 px-5 py-4 backdrop-blur lg:px-9">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className="rounded-xl border-[#d2e9e3] bg-white text-xs font-medium text-[#267b6b] hover:bg-[#ebf8f4] flex items-center gap-1.5 h-8 px-2.5"
          >
            <ShieldCheck className="size-3.5 text-[#238f76]" />
            <span className="hidden sm:inline">โปรไฟล์แอดมิน</span>
          </Button>
          <Button
            variant="outline"
            onClick={handleLogout}
            className="rounded-xl border-[#f5d5cf] bg-white text-xs font-medium text-[#c04332] hover:bg-[#fff5f3] flex items-center gap-1.5 h-8 px-2.5"
          >
            <LogOut className="size-3.5 text-[#c04332]" />
      ...
      {/* Admin Profile Management Modal */}
      {showAdminProfileModal && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-[2rem] bg-white p-7 text-left shadow-2xl border border-[#d2ebe5] paper-shadow animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between border-b border-[#e6f3ef] pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-display text-base font-bold text-[#1a3832]">จัดการโปรไฟล์ผู้ดูแลระบบ</h3>
                  <p className="text-[11px] text-[#248e75] font-semibold">Milo System Administrator</p>
                </div>
              </div>
              <button
                onClick={() => setShowAdminProfileModal(false)}
                className="grid size-8 place-items-center rounded-xl text-[#789e96] hover:bg-[#f0faf7] hover:text-[#1e4840]"
              >
                ✕
              </button>
            </div>

            <div className="mt-5 space-y-3.5 text-xs text-[#305e54]">
              <div className="rounded-2xl bg-[#f5fbf9] p-4 border border-[#e4f4ee] space-y-2">
                <div className="flex justify-between">
                  <span className="text-[#6e948c]">ชื่อผู้ใช้ (Username):</span>
                  <span className="font-semibold text-[#1c4d44] font-mono">admin</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#6e948c]">สิทธิ์ในระบบ (Role):</span>
                  <span className="font-semibold text-[#187863] bg-[#ddf7ed] px-2 py-0.5 rounded-full text-[10px]">Super Admin</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#6e948c]">สถานะ LINE Bot:</span>
                  <span className={`font-semibold ${isLinked ? "text-[#187863]" : "text-[#b27129]"}`}>
                    {isLinked ? "เชื่อมต่อเรียบร้อย" : "ยังไม่ได้เชื่อมต่อ"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#6e948c]">ฐานข้อมูลคลาวด์:</span>
                  <span className="font-medium text-[#2d5f56]">TiDB Cloud (Singapore)</span>
                </div>
              </div>

              <div className="rounded-2xl border border-[#e3f0ec] bg-white p-4 space-y-1.5">
                <p className="font-semibold text-[#255249]">การเปลี่ยนรหัสผ่านแอดมิน</p>
                <p className="text-[11px] leading-relaxed text-[#688d85]">
                  เพื่อความปลอดภัยสูงสุด สามารถกำหนดรหัสผ่านใหม่ได้ทันทีผ่านตัวแปร <strong>ADMIN_PASSWORD</strong> ใน Vercel Settings &gt; Environment Variables
                </p>
              </div>
            </div>

            <div className="mt-6 flex items-center justify-between gap-3 pt-4 border-t border-[#eaf4f1]">
              <Button
                variant="outline"
                onClick={handleLogout}
                className="rounded-xl border-[#f2d2cc] bg-white text-xs font-semibold text-[#be4130] hover:bg-[#fff4f2] flex items-center gap-1.5 px-4 h-10"
              >
