...
const PRODUCTION_DASHBOARD_URL = "https://milo-line-assistant.onrender.com/dashboard";
...
export default function Dashboard() {
  ...
  const financeAnalytics = trpc.milo.finance.analytics.useQuery(financeScopeInput, { enabled: Boolean(overview.data?.lineUserId || linkedLineUserId) && Boolean(activeFinanceAccountId) && overview.data?.plan !== "free" });
  ...
  useEffect(() => { setTransactionPage(0); }, [activeFinanceAccountId, transactionSearch, transactionTypeFilter, transactionCategoryFilter]);
  ...
  const financeDigestStatus = trpc.milo.automation.financeDigestStatus.useQuery(undefined, { enabled: Boolean(overview.data?.lineUserId || linkedLineUserId) });
  const setFinanceDigestEnabled = trpc.milo.automation.setFinanceDigestEnabled.useMutation({
    onSuccess: () => { toast.success("อัปเดตสรุปการเงินอัตโนมัติแล้ว"); void financeDigestStatus.refetch(); },
    onError: error => toast.error(error.message),
  });
  ...
  const canUseProFeatures = data.plan === "pro" || data.plan === "pro_max";
  const canUseProMaxFeatures = data.plan === "pro_max";
  ...
  const scopedAnalytics = canUseProFeatures ? (activeFinanceAccountId ? (financeAnalytics.data ?? { daily: [], transactionCount: 0, sevenDayIncome: 0, sevenDayExpense: 0 }) : data.financeAnalytics) : { daily: [], transactionCount: 0, sevenDayIncome: 0, sevenDayExpense: 0 };
  ...
  const shownVault: typeof vaultItemsForManagement = vaultItemsForManagement;
  const allScopedTransactions = transactions.data ?? [];
  const transactionCategories = Array.from(new Set(allScopedTransactions.map(item => item.category))).sort((a, b) => a.localeCompare(b, "th"));
  const filteredTransactions = filterFinanceTransactions(allScopedTransactions, transactionSearch, { transactionType: transactionTypeFilter, category: transactionCategoryFilter });
  ...
  const paginatedTransactions = filteredTransactions.slice(transactionWindow.start, transactionWindow.end);
  ...
  return <div className={`relative min-h-screen bg-[#f5fcfa] text-[#245851] ${!isProductionSite ? "preview-surface" : ""}`}>
    ...
    <aside className="fixed inset-y-0 hidden w-64 flex-col border-r border-[#d6ece6] bg-white p-5 lg:flex overflow-y-auto">
    ...
    <main className="relative lg:ml-64" aria-busy={contentMutationPending}>
      ...
      <header className="sticky top-0 z-20 flex items-center justify-between border-b border-[#d8ede8] bg-[#f5fcfa]/90 px-5 py-4 backdrop-blur lg:px-9">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isLinked && (
            <button type="button" onClick={() => setIsEditingLineLink(true)} className="grid size-8 place-items-center rounded-lg border border-[#d2e9e3] bg-white text-[#548078] transition-colors hover:bg-[#edf8f4]" aria-label="แก้ไขบัญชี LINE" title="แก้ไขบัญชี LINE">
              <Pencil className="size-3.5" />
            </button>
          )}
          <Button type="button" variant="outline" size="sm" onClick={() => setShowAdminProfileModal(true)} className="rounded-xl border-[#cce4dd] bg-white text-xs text-[#238a72] hover:bg-[#f0faf6] font-medium">
            <UserCog className="mr-1.5 size-3.5 text-[#238a72]" />
            โปรไฟล์แอดมิน
          </Button>
          <Button type="button" variant="outline" size="sm" onClick={logout} className="rounded-xl border-[#f2d5ce] bg-white text-xs text-[#b44b3e] hover:bg-[#fff5f3] font-medium">
            <LogOut className="mr-1.5 size-3.5 text-[#b44b3e]" />
            ออกจากระบบ
          </Button>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-7xl p-5 lg:p-9">
        {!isLinked || isEditingLineLink ? <LinkAccountForm lineUserId={lineUserId} setLineUserId={setLineUserId} pending={link.isPending} isRelinking={isLinked} onCancel={isLinked ? () => { setIsEditingLineLink(false); setLineUserId(""); } : undefined} onSubmit={() => link.mutate({ lineUserId: lineUserId.trim() })} /> : <>
          <section id="overview" className="scroll-mt-24 grid gap-4 sm:grid-cols-2 xl:grid-cols-[1.3fr_1fr_1fr_1fr]"><StatCard label="ยอดคงเหลือในบัญชี" value={money.format(scopedFinance.balance)} icon={BarChart3} tone="violet" emphasized /><StatCard label="รายรับเดือนนี้" value={money.format(scopedFinance.income)} icon={TrendingUp} tone="mint" /><StatCard label="รายจ่ายเดือนนี้" value={money.format(scopedFinance.expense)} icon={ReceiptText} tone="peach" /><StatCard label={canUseProFeatures ? "ธุรกรรมใน 7 วัน" : "แพ็กเกจปัจจุบัน"} value={canUseProFeatures ? `${scopedAnalytics.transactionCount} รายการ` : data.planCapabilities.label} icon={FileText} tone="sky" /></section>

          <DashboardDetailMenu data={{ ...data, finance: scopedFinance, budgets: scopedBudgets }} transactions={transactions.data ?? []} onNavigate={goTo} allowPdfExport={canUseProMaxFeatures} />

          {canUseProFeatures ? <FinanceAnalyticsPanel trend={trendChart} categories={categoryChart} transactionCount={scopedAnalytics.transactionCount} sevenDayIncome={scopedAnalytics.sevenDayIncome} sevenDayExpense={scopedAnalytics.sevenDayExpense} /> : <PlanGatePanel id="finance" title="กราฟวิเคราะห์ขั้นสูง" requiredPlan="Pro" description="อัปเกรดเป็น Pro เพื่อดูกราฟแนวโน้ม รายรับ–รายจ่าย และสัดส่วนรายจ่ายจากข้อมูลจริง" />}
          ...
          <FinanceSettingsPanel financeAccountId={activeFinanceAccountId} role={activeFinanceAccount?.membership.role} allowCustomBudgetCycle={canUseProFeatures} />
          <FinanceDigestAutomationPanel data={financeDigestStatus.data} loading={financeDigestStatus.isLoading} error={financeDigestStatus.error?.message} isAdmin={user.role === "admin"} pending={setFinanceDigestEnabled.isPending} onRetry={() => void financeDigestStatus.refetch()} onToggle={(digestType, enabled) => setFinanceDigestEnabled.mutate({ digestType, enabled })} />
          <FinancialAssistantPanel financeAccountId={activeFinanceAccountId} />
          <TransactionSearch value={transactionSearch} matchedCount={filteredTransactions.length} totalCount={allScopedTransactions.length} transactionType={transactionTypeFilter} category={transactionCategoryFilter} categories={transactionCategories} onChange={setTransactionSearch} onTransactionTypeChange={setTransactionTypeFilter} onCategoryChange={setTransactionCategoryFilter} />
          ...
          {canUseProMaxFeatures ? <FinanceAccountManagerPanel groups={data.groups} /> : <PlanGatePanel id="groups" title="LINE Group Accounting" requiredPlan="Pro Max" description="การทำบัญชีในกลุ่ม LINE และสมุดบัญชีหลายบัญชีเปิดใช้ใน Pro Max" />}
      ...
      {showAdminProfileModal && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-3xl bg-white p-6 text-left shadow-2xl border border-[#d3ebe5]">
            <div className="flex items-center justify-between border-b border-[#edf4f1] pb-3">
              <div className="flex items-center gap-2.5">
                <div className="grid size-10 place-items-center rounded-xl bg-[#e4f7f1] text-[#1c8c72]">
                  <UserCog className="size-5" />
                </div>
                <div>
                  <h3 className="text-base font-bold text-[#1a3832]">โปรไฟล์ผู้ดูแลระบบ</h3>
                  <p className="text-[11px] text-[#638b82]">Milo Administrator Profile</p>
                </div>
              </div>
              <span className="rounded-full bg-[#dff8e9] px-2.5 py-1 text-[11px] font-semibold text-[#187e67]">
                Admin (สิทธิ์สูงสุด)
              </span>
            </div>

            <div className="mt-4 space-y-3 text-xs">
              <div className="rounded-xl bg-[#f7fbf9] p-3 border border-[#e2f1ec] space-y-2">
                <div className="flex justify-between">
                  <span className="text-[#6d918a]">ชื่อผู้ดูแล:</span>
                  <span className="font-semibold text-[#22584e]">{user.name || "ผู้ดูแลระบบ (Admin)"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#6d918a]">Username:</span>
                  <span className="font-mono font-semibold text-[#22584e]">admin</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#6d918a]">บทบาท (Role):</span>
                  <span className="font-semibold text-[#1c8c72]">{user.role}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#6d918a]">บัญชี LINE ที่เชื่อม:</span>
                  <span className="font-mono text-[#285d54] text-[11px] truncate max-w-[200px]" title={data.lineUserId || "ยังไม่ได้เชื่อม"}>
                    {data.lineUserId || "ยังไม่ได้เชื่อม"}
                  </span>
                </div>
              </div>

              <div className="rounded-xl bg-[#fffaf2] p-3 border border-[#f5e6d0] text-[#7c5b24]">
                <p className="font-semibold text-xs flex items-center gap-1.5">
                  <ShieldCheck className="size-3.5 text-[#b97a22]" /> การเปลี่ยนรหัสผ่านผู้ดูแลระบบ:
                </p>
                <p className="mt-1 text-[11px] leading-relaxed text-[#926c2e]">
                  คุณสามารถเปลี่ยน Username หรือรหัสผ่านใหม่ได้ตลอดเวลาผ่านตัวแปร ADMIN_USERNAME และ ADMIN_PASSWORD ใน Vercel Dashboard
                </p>
              </div>
            </div>

            <div className="mt-5 flex items-center justify-between border-t border-[#edf4f1] pt-3">
              <Button
                type="button"
                variant="outline"
                size="sm"
                onClick={logout}
                className="rounded-xl border-[#f2d5ce] text-xs text-[#b44b3e] hover:bg-[#fff5f3] font-medium"
              >
                <LogOut className="mr-1.5 size-3.5 text-[#b44b3e]" />
                ออกจากระบบ
              </Button>
...
function FinanceDigestAutomationPanel({ data, loading, error, isAdmin, pending, onRetry, onToggle }: { data?: { settings: Array<{ settingKey: string; isEnabled: boolean; lastRunAt: Date | string | null; updatedAt: Date | string }>; deliveries: Array<{ id: number; digestType: "daily" | "weekly"; periodKey: string; status: "sending" | "sent" | "failed"; createdAt: Date | string; finishedAt: Date | string | null; errorMessage: string | null }> }; loading: boolean; error?: string; isAdmin: boolean; pending: boolean; onRetry: () => void; onToggle: (digestType: "daily" | "weekly", enabled: boolean) => void }) {
  const schedules = [{ digestType: "daily" as const, settingKey: "finance-digest-daily", label: "สรุปรายวัน", schedule: "ทุกวัน 06:00 น." }, { digestType: "weekly" as const, settingKey: "finance-digest-weekly", label: "สรุปรายสัปดาห์", schedule: "ทุกวันจันทร์ 06:00 น." }];
  if (loading) return <section className="mt-6 rounded-[1.6rem] border border-[#d7ebe6] bg-white p-6 soft-shadow"><div className="h-24 animate-pulse rounded-2xl bg-[#f3faf8]" aria-label="กำลังโหลดสถานะสรุปการเงินอัตโนมัติ" /></section>;
  if (error) return <section className="mt-6 rounded-[1.6rem] border border-[#f0d6cf] bg-white p-6 soft-shadow"><p className="text-sm font-semibold text-[#9d564b]">โหลดสถานะสรุปอัตโนมัติไม่สำเร็จ</p><p className="mt-1 text-xs text-[#9a7a73]">{error}</p><Button type="button" variant="outline" size="sm" onClick={onRetry} className="mt-3 rounded-lg">ลองใหม่</Button></section>;
  const settings = data?.settings ?? [];
  const deliveries = data?.deliveries ?? [];
  const deliveryLabel = (status: "sending" | "sent" | "failed") => status === "sent" ? "สำเร็จ" : status === "failed" ? "ล้มเหลว" : "กำลังส่ง";
  return <section className="mt-6 rounded-[1.6rem] border border-[#d7ebe6] bg-white p-6 soft-shadow" aria-label="สรุปการเงินอัตโนมัติ"><SectionTitle title="สรุปการเงินอัตโนมัติ" subtitle="สถานะ scheduler และประวัติการส่งจริงจาก delivery audit" icon={Clock3} tone="text-[#765aa0]" /><div className="mt-4 grid gap-3 md:grid-cols-2">{schedules.map(item => { const setting = settings.find(row => row.settingKey === item.settingKey); const latest = deliveries.find(row => row.digestType === item.digestType); const enabled = Boolean(setting?.isEnabled); return <div key={item.settingKey} className="rounded-2xl border border-[#e5def0] bg-[#fdfbff] p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold text-[#60477d]">{item.label}</p><p className="mt-1 text-xs text-[#907aa6]">{item.schedule} · Asia/Bangkok</p></div><span className={"rounded-full px-2.5 py-1 text-[10px] font-semibold " + (enabled ? "bg-[#e5f7ec] text-[#267d58]" : "bg-[#f1f1f1] text-[#777]")}>{setting ? enabled ? "เปิดใช้งาน" : "ปิดอยู่" : "ยังไม่ตั้งค่า"}</span></div><div className="mt-3 text-[11px] leading-5 text-[#7f7190]"><p>รันล่าสุด: {setting?.lastRunAt ? dateTime.format(new Date(setting.lastRunAt)) : "ยังไม่มีข้อมูล"}</p><p>ส่งล่าสุด: {latest ? deliveryLabel(latest.status) + " · " + latest.periodKey : "ยังไม่มีประวัติ"}</p></div>{isAdmin && setting && <Button type="button" size="sm" variant="outline" disabled={pending} onClick={() => onToggle(item.digestType, !enabled)} className="mt-3 h-8 rounded-lg border-[#d9cbea] text-xs text-[#70528e]">{pending ? "กำลังอัปเดต..." : enabled ? "ปิดชั่วคราว" : "เปิดใช้งาน"}</Button>}</div>; })}</div><div className="mt-4 rounded-2xl bg-[#f7fbfa] p-4"><div className="flex items-center justify-between gap-2"><p className="text-xs font-semibold text-[#47766d]">ประวัติการส่งล่าสุด</p><span className="text-[10px] text-[#86a19b]">{deliveries.length} รายการ</span></div>{deliveries.length ? <div className="mt-2 space-y-2">{deliveries.slice(0, 6).map(row => <div key={row.id} className="flex flex-wrap items-center gap-2 rounded-xl bg-white px-3 py-2 text-[11px]"><span className={"rounded-full px-2 py-0.5 font-semibold " + (row.status === "sent" ? "bg-[#e5f7ec] text-[#267d58]" : row.status === "failed" ? "bg-[#fff0ec] text-[#b85e52]" : "bg-[#eef4fb] text-[#4a7698]")}>{row.status === "sent" ? "ส่งแล้ว" : row.status === "failed" ? "ล้มเหลว" : "กำลังส่ง"}</span><span className="font-medium text-[#4f7069]">{row.digestType === "daily" ? "รายวัน" : "รายสัปดาห์"}</span><span className="text-[#789891]">{row.periodKey}</span><span className="ml-auto text-[#8da49f]">{dateTime.format(new Date(row.finishedAt ?? row.createdAt))}</span>{row.errorMessage && <span className="w-full text-[#b85e52]">{row.errorMessage}</span>}</div>)}</div> : <Empty text="ยังไม่มี delivery audit ของสรุปการเงินอัตโนมัติ" />}</div></section>;
}
...
function TransactionSearch({ value, matchedCount, totalCount, transactionType, category, categories, onChange, onTransactionTypeChange, onCategoryChange }: { value: string; matchedCount: number; totalCount: number; transactionType: "all" | "income" | "expense"; category: string; categories: string[]; onChange: (value: string) => void; onTransactionTypeChange: (value: "all" | "income" | "expense") => void; onCategoryChange: (value: string) => void }) {
  return <section id="transactions-main" className="scroll-mt-24 mt-6 rounded-[1.35rem] border border-[#dceee9] bg-[#fbfefd] p-4" aria-label="ค้นหาและกรองธุรกรรม"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-[#315f58]">ค้นหาและกรองธุรกรรมย้อนหลัง</p><p className="mt-1 text-xs text-[#789891]">ค้นจากจำนวนเงิน หมวด หรือหมายเหตุ แล้วกรองรายรับ/รายจ่ายและหมวดในสมุดบัญชีที่เลือก</p></div><span className="text-xs text-[#5d897e]">พบ {matchedCount} จาก {totalCount} รายการ</span></div><div className="mt-3 grid gap-2 md:grid-cols-[1fr_150px_190px]"><Input value={value} onChange={event => onChange(event.target.value)} placeholder="เช่น กาแฟ, 65 หรือ เงินเดือน" aria-label="ค้นหาธุรกรรมย้อนหลัง" className="rounded-xl bg-white" /><select value={transactionType} onChange={event => onTransactionTypeChange(event.target.value as "all" | "income" | "expense")} aria-label="กรองประเภทธุรกรรม" className="h-10 rounded-xl border border-[#d3e8e3] bg-white px-3 text-xs text-[#527d74]"><option value="all">ทุกประเภท</option><option value="income">รายรับ</option><option value="expense">รายจ่าย</option></select><select value={category} onChange={event => onCategoryChange(event.target.value)} aria-label="กรองหมวดธุรกรรม" className="h-10 rounded-xl border border-[#d3e8e3] bg-white px-3 text-xs text-[#527d74]"><option value="">ทุกหมวด</option>{categories.map(item => <option key={item} value={item}>{item}</option>)}</select></div>{(value || transactionType !== "all" || category) && <button type="button" onClick={() => { onChange(""); onTransactionTypeChange("all"); onCategoryChange(""); }} className="mt-2 text-[11px] font-medium text-[#2b876f] hover:underline">ล้างตัวกรอง</button>}</section>;
}
...
function DashboardDetailMenu({ data, transactions, onNavigate, allowPdfExport }: { data: DashboardDetailData; transactions: DashboardTransaction[]; onNavigate: (id: DashboardSectionId) => void; allowPdfExport: boolean }) {
...
function PlanGatePanel({ id, title, requiredPlan, description }: { id: string; title: string; requiredPlan: "Pro" | "Pro Max"; description: string }) { return <section id={id} className="scroll-mt-24 mt-6 rounded-[1.6rem] border border-dashed border-[#d8cdea] bg-[#fcf9ff] p-6 soft-shadow" aria-label={`${title} ต้องใช้แพ็กเกจ ${requiredPlan}`}><div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-2xl bg-[#f0eaff] text-[#765aa0]"><ShieldCheck className="size-5" /></span><div><p className="text-sm font-semibold text-[#4f3b6f]">{title}</p><p className="mt-1 text-xs leading-5 text-[#7f7190]">{description}</p><span className="mt-3 inline-flex rounded-full bg-white px-3 py-1.5 text-[11px] font-semibold text-[#76519e] shadow-sm">ต้องใช้ {requiredPlan}</span></div></div></section>; }
        ...
        {showForgotModal && (
          <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 backdrop-blur-sm p-4">
            <div className="w-full max-w-sm rounded-3xl bg-white p-6 text-left shadow-2xl border border-[#d3ebe5]">
              <h3 className="text-base font-bold text-[#1a3d36]">กู้คืนรหัสผ่านผู้ดูแลระบบ</h3>
