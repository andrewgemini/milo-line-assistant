...
describe("Milo command parser", () => {
  ...
  it("recognizes transaction search, update, delete, and financial report commands", () => {
    expect(parseMiloCommand("ค้นหารายการ กาแฟ", now)).toEqual({ type: "transactionSearch", query: "กาแฟ" }); expect(parseMiloCommand("ลบรายการ 42", now)).toEqual({ type: "transactionDelete", id: 42 }); expect(parseMiloCommand("แก้รายการ 42 เป็น 180", now)).toEqual({ type: "transactionUpdate", id: 42, amount: 180 }); expect(parseMiloCommand("สรุป", now)).toEqual({ type: "financeReport", period: "year" }); expect(parseMiloCommand("สรุปเดือนนี้", now)).toEqual({ type: "financeReport", period: "month" }); expect(parseMiloCommand("สรุปการเงินวันนี้", now)).toEqual({ type: "financeReport", period: "day" }); expect(parseMiloCommand("ตั้งยอดเงินเริ่มต้น 5000 บาท", now)).toEqual({ type: "openingBalance", amount: 5000 });
  });
