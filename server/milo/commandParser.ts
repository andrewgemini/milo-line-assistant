...
export function parseMiloCommand(text: string, now = new Date()): MiloCommand {
  ...
  const financeReport = value.match(/^สรุป(?:การเงิน|รายรับรายจ่าย|ยอด(?:ประจำเดือน)?)?\s*(?:ของ)?\s*(วันนี้|สัปดาห์นี้|เดือนนี้|ปีนี้)?$/i); if (financeReport) { const periodKey = financeReport[1] ?? "ปีนี้"; const periods: Record<string, "day" | "week" | "month" | "year"> = { "วันนี้": "day", "สัปดาห์นี้": "week", "เดือนนี้": "month", "ปีนี้": "year" }; return { type: "financeReport", period: periods[periodKey] ?? "year" }; }
