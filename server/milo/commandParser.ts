...
function titleWithoutSchedule(text: string) {
  return text.replace(/(?:ทุก\s*\d+\s*นาที|ทุกวัน|ทุกสัปดาห์(?:วัน)?(?:อาทิตย์|จันทร์|อังคาร|พุธ|พฤหัส|ศุกร์|เสาร์)?|ทุกเดือน(?:วันที่)?\s*\d+|พรุ่งนี้|วันนี้|วันที่\s*\d+\/\d+(?:\/\d+)?|\d{4}-\d{1,2}-\d{1,2}|(?:เวลา\s*)?\d{1,2}(?::|\.)?\d{0,2}\s*น?\.?)/gi, "").replace(/\s+/g, " ").trim() || "รายการเตือน";
}

function clock(text: string) {
  const match = text.match(/(?:เวลา\s*)?(\d{1,2})(?:(?::|\.)(\d{2}))?\s*(?:น\.?|โมง)?/i);
...
function reminderFrom(text: string, now: Date): ReminderDraft | undefined {
  ...
  const body = text.trim().replace(/^(?:@?ไมโล\s*)?(?:ตั้ง)?เตือน(?:ฉัน)?\s*/i, "");
  ...
  if (minutes) { const interval = Math.max(1, Number(minutes[1])); const run = new Date(now.getTime() + interval * 60_000); return { title, recurrenceType: "minute", recurrenceInterval: interval, dueAt: run, nextRunAt: run }; }
  ...
  if (weekly) { const map: Record<string, number> = { "อาทิตย์": 0, "จันทร์": 1, "อังคาร": 2, "พุธ": 3, "พฤหัส": 4, "ศุกร์": 5, "เสาร์": 6 }; const weekday = map[weekly[1] ?? "จันทร์"]; const parts = bangkokParts(now); const days = (weekday - parts.weekday + 7) % 7 || 7; const date = addBangkokDays(parts, days); const run = atBangkok(date.year, date.month, date.day, time.hour, time.minute); return { title, recurrenceType: "week", recurrenceInterval: 1, recurrenceWeekdays: String(weekday), dueAt: run, nextRunAt: run }; }
  const monthly = body.match(/ทุกเดือน(?:วันที่)?\s*(\d{1,2})?/i);
  if (monthly) { const parts = bangkokParts(now); const day = Math.min(Math.max(Number(monthly[1] ?? parts.day), 1), 28); let run = atBangkok(parts.year, parts.month, day, time.hour, time.minute); if (run <= now) run = atBangkok(parts.year, parts.month + 1, day, time.hour, time.minute); return { title, recurrenceType: "month", recurrenceInterval: 1, recurrenceDayOfMonth: day, dueAt: run, nextRunAt: run }; }
  const parts = bangkokParts(now);
  let run = atBangkok(parts.year, parts.month, parts.day, time.hour, time.minute);
  if (/พรุ่งนี้/i.test(body)) { const tomorrow = addBangkokDays(parts, 1); run = atBangkok(tomorrow.year, tomorrow.month, tomorrow.day, time.hour, time.minute); }
  ...
  if (thai) { const rawYear = thai[3] ? Number(thai[3]) : parts.year; const year = rawYear > 2400 ? rawYear - 543 : rawYear; run = atBangkok(year, Number(thai[2]), Number(thai[1]), time.hour, time.minute); if (!thai[3] && run <= now) run = atBangkok(year + 1, Number(thai[2]), Number(thai[1]), time.hour, time.minute); }
...
export function parseMiloCommand(text: string, now = new Date()): MiloCommand {
  ...
  if (money) { const income = /รับ|รายรับ/i.test(money[1]); const note = money[2].trim(); const transactionType = income ? "income" : "expense"; return { type: transactionType, amount: Number(money[3].replace(/,/g, "")), category: suggestStandardCategory(transactionType, note), note }; }
  ...
  const note = value.match(/^(โน้ต|บันทึก)\s+(.+)$/i); if (note) return { type: "note", title: note[2].slice(0, 80), content: note[2] };
  const todo = value.match(/^(งาน|todo|ทูดู)\s+(.+)$/i); if (todo) return { type: "todo", title: todo[2] };
  const vault = value.match(/^(เก็บ|บันทึกไว้)\s+(.+)$/i);
  if (vault) {
    const content = vault[2].trim();
    const sourceUrl = content.match(/https?:\/\/\S+/i)?.[0];
    const tagsText = (content.match(/#[^\s#]+/g) ?? []).join(" ") || undefined;
    return { type: "vault", title: (sourceUrl ?? content).slice(0, 80), content, itemType: sourceUrl ? "link" : "text", sourceUrl, tagsText };
  }
  const search = value.match(/^(ค้นหา|หาไฟล์|ค้น)\s+(.+)$/i); if (search) return { type: "search", query: search[2] };
  const mention = value.match(/^แจ้ง\s*(.+?)\s*ถึง\s*@?(.+)$/i); if (mention) return { type: "mention", message: mention[1].trim(), memberName: mention[2].trim() };
  ...
  if (categoryList) return { type: "categoryList", transactionType: categoryList[1] === "รายรับ" ? "income" : categoryList[1] === "รายจ่าย" ? "expense" : undefined };
  const budget = value.match(/^(?:ตั้ง)?งบ\s+(.+?)\s+(\d[\d,]*(?:\.\d{1,2})?)\s*(?:บาท)?$/i);
  if (budget) {
    const category = budget[1].trim(); const amount = Number(budget[2].replace(/,/g, ""));
