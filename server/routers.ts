  ...
  milo: router({
    ...
    finance: router({
      ...
      categories: router({
        ...
        remove: protectedProcedure.input(z.object({ name: z.string().trim().min(1).max(100), transactionType: z.enum(["income", "expense"]), financeAccountId: z.number().int().positive().optional() })).mutation(async ({ ctx, input }) => { const scope = await requireFinanceAccountScope(ctx.user.id, input.financeAccountId); requireFinancePermission(db.canManageFinanceSettings(scope.role), "สิทธิ์ของคุณยังจัดการหมวดในสมุดบัญชีนี้ไม่ได้"); const removed = await db.removeExpenseCategory(scope.lineUserId, input.name, input.transactionType, scope.financeAccountId); if (!removed) throw new Error("ไม่พบหมวดที่ต้องการลบ"); await db.writeAuditLog({ action: "finance_category.delete", entityType: "expense_category", dashboardUserId: ctx.user.id, actorLineUserId: scope.lineUserId, lineChatId: scope.account.lineChatId ?? undefined, details: { financeAccountId: scope.financeAccountId, name: input.name, transactionType: input.transactionType } }); return { success: true } as const; }),
      }),
    }),
