  ...
  auth: router({
    ...
    adminLogin: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        return { success: true, token: sessionToken, user: { openId: safeOpenId, name, role: "admin" } };
      }),
  }),
  milo: router({
    ...
    admin: router({
      ...
      updateAdminProfile: protectedProcedure.input(z.object({ name: z.string().trim().min(1).optional(), newPassword: z.string().min(4, "รหัสผ่านต้องมีอย่างน้อย 4 ตัวอักษร").optional() })).mutation(async ({ ctx, input }) => {
        requireAdminRole(ctx.user.role);
        if (input.name) {
          try {
            await db.upsertUser({ openId: ctx.user.openId, name: input.name, role: "admin", lastSignedIn: new Date() });
          } catch {}
        }
