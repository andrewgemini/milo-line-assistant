...
auth: router({
  ...
  milo: router({
    linkLineAccount: protectedProcedure.input(z.object({ lineUserId: z.string().trim().regex(/^U[0-9a-fA-F]{32}$/, "LINE User ID ??????????????? U ???????????????? 32 ???") })).mutation(async ({ ctx, input }) => {
    ...
    financeAccounts: router({
      ...
      upsertMember: protectedProcedure.input(z.object({ financeAccountId: z.number().int().positive(), lineUserId: z.string().trim().regex(/^U[0-9a-fA-F]{32}$/, "LINE User ID ??????????????? U ???????????????? 32 ???"), role: z.enum(["manager", "contributor", "viewer"]) })).mutation(async ({ ctx, input }) => {
        const scope = await requireFinanceAccountScope(ctx.user.id, input.financeAccountId);
        requireFinancePermission(db.canManageFinanceAccountMembers(scope.role), "???????????????????????????????????????");
        if (!await db.isEligibleGroupFinanceAccountMember(input.financeAccountId, input.lineUserId)) throw new Error("??????????????????????????????????????????? LINE ???");
      ...
      removeMember: protectedProcedure.input(z.object({ financeAccountId: z.number().int().positive(), lineUserId: z.string().trim().regex(/^U[0-9a-fA-F]{32}$/, "LINE User ID ??????????????? U ???????????????? 32 ???") })).mutation(async ({ ctx, input }) => {
        const scope = await requireFinanceAccountScope(ctx.user.id, input.financeAccountId);
        requireFinancePermission(db.canManageFinanceAccountMembers(scope.role), "???????????????????????????????????????");
    ...
    finance: router({
      ...
      openingBalance: protectedProcedure.input(z.object({ amount: z.number().min(0), effectiveAt: z.coerce.date().optional(), financeAccountId: z.number().int().positive().optional() })).mutation(async ({ ctx, input }) => {
        const scope = await requireFinanceAccountScope(ctx.user.id, input.financeAccountId);
        requireFinancePermission(db.canManageFinanceSettings(scope.role), "?????????????????????????????????????????????????????");
      ...
      create: protectedProcedure.input(z.object({ transactionType: z.enum(["income", "expense"]), amount: z.number().positive(), category: z.string().trim().min(1).max(100), note: z.string().trim().max(2000).optional(), occurredAt: z.coerce.date().optional(), financeAccountId: z.number().int().positive().optional() })).mutation(async ({ ctx, input }) => {
        const scope = await requireFinanceAccountScope(ctx.user.id, input.financeAccountId);
        requireFinancePermission(db.canCreateFinanceTransaction(scope.role), "??????????????????????????????????? ???????????????????????");
      ...
      update: protectedProcedure.input(z.object({ id: z.number().int().positive(), transactionType: z.enum(["income", "expense"]).optional(), amount: z.number().positive().optional(), category: z.string().trim().min(1).max(100).optional(), note: z.string().trim().max(2000).nullable().optional(), occurredAt: z.coerce.date().optional(), financeAccountId: z.number().int().positive().optional() })).mutation(async ({ ctx, input }) => {
        const scope = await requireFinanceAccountScope(ctx.user.id, input.financeAccountId);
        requireFinancePermission(db.canManageFinanceTransactions(scope.role), "??????????????????????????????????????????????");
