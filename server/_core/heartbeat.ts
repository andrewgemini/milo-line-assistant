...
const HEARTBEAT_REQUEST_TIMEOUT_MS = 20_000;
...
const callForge = async <T>(
  ...
  try {
    ...
    const timeoutMessage = error instanceof DOMException && error.name === "TimeoutError"
      ? `Heartbeat ${rpc} timed out after ${HEARTBEAT_REQUEST_TIMEOUT_MS / 1000} seconds`
