...
const richMenuConfig = { size: { width: 2500, height: 1686 }, selected: true, name: "Milo LINE Assistant - Master Rich Menu", chatBarText: "เมนูไมโล", areas: [
...
{ bounds: { x: 590, y: 1030, width: 560, height: 656 }, action: messageAction("ตั้งค่า", "ตั้งค่า") },
...
async function main() {
 ...
 const create = await api("/v2/bot/richmenu", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(richMenuConfig) }); if (!create.ok) throw new Error(`create richmenu failed (${create.status}) ${await create.text()}`); const menu = await create.json();
